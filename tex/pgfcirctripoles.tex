...
\long\def\pgfcircdeclaretransistor#1#2#3{
    \pgfdeclareshape{#1}
    {
        ...
        %cache the value for later use in text
        \savedmacro{\legacyTextActive}{
            \ifpgf@circuit@transisors@fixlabels%
                \edef\legacyTextActive{1}
            \else
                \edef\legacyTextActive{0}
            \fi
        }
        \anchor{text}{
            \northeast
            \pgfmathsetlength{\pgf@circ@scaled@Rlen}{\ctikzvalof{\ctikzclass/scale}\pgf@circ@Rlen}
            \ifnum \legacyTextActive>0
                \pgfmathsetlength{\pgf@x}{\pgf@x+\extrabodydiodelen}
                % if a circle exists (i.e. circle radius>0), the text position should be outside the circle
                \pgfmathsetlength{\pgf@x}{max(\circleleft+2*\circleradius,\pgf@x)}
